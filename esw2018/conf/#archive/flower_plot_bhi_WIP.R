...
apply_bhi_theme <- function(plot_type = NA){
  ...
  bhi_palettes <- list(
    ...
    goals_pal = tibble::tibble(
      goal = c("MAR","FIS","SPP","HAB","CW","LSP","ICO","ECO","LIV","TR","CP","CS","AO"),
      color = c("#9A1A4D", "#D84B5B", "#F06352", "#F77753", "#F6955E", "#FFCC81", "#F3E48F", "#CAE297", "#A9D7A4", "#6AC2A7", "#51ACB1", "#3B8DBF", "#5D56A2"))
    )

  #--------------#
  #9 goals
  #goal_colours <- c("#BB3153", "#F06352", "#F77753", "#F6955E", "#FFCC81", "#E4E999", "#88CCA5", "#51ACB1", "#4E70B7")
  #slices <- rep.int(100/9, 9)
  #pie(slices, col = goal_colours)
  #13 sub goals
  #subgoal_colours <- c("#9A1A4D", "#D84B5B", "#F06352", "#F77753", "#F6955E", "#FFCC81", "#F3E48F", "#CAE297", "#A9D7A4", "#6AC2A7", "#51ACB1", "#3B8DBF", "#5D56A2")
  #slices <- rep.int(100/13, 13)
  #pie(slices, col = subgoal_colours)
  #--------------#
  
  ## region names lookup table ----
  rgn_name_lookup <- rbind(
    readr::read_csv("spatial/regions_list.csv") %>%
      dplyr::select(region_id = rgn_id, rgn_name),
    data.frame(region_id = 0,rgn_name = "Southwest England")) %>%
    dplyr::mutate(plot_title = ifelse(region_id != 0, paste0(region_id, ", ", rgn_name), rgn_name)) %>%
    rowwise() %>%
...
color2 = c("#9A1A4D", "#D84B5B", "#F06352", "#F77753", "#F6955E", "#FFCC81", "#F3E48F", "#CAE297", "#A9D7A4", "#6AC2A7", "#51ACB1", "#3B8DBF", "#5D56A2")
...
make_flower_plot <- function(rgn_scores, rgn_id = NA, plot_year = NA, dim = "score",
  ...
  rgn_scores <- read.csv("scores.csv")
  rgn_id = 1 
  plot_year = 2018 
  dim = "score"
  color_pal = color2 
...
#turn off  
  #w_fn <- list.files(file.path(dir_baltic, "layers"),
                     ...
                     #full.names = TRUE)
#SKP from 'flower_plot'
#following code can be turned off
#and weights added manually
#--------------#
...
#SKP  
  plot_config <-  readr::read_csv('conf/goals.csv')
  ...
  #sets up text for curved labels
  ...
  if(color_by == "goal"){
    if(length(plot_config$goal) > length(color_pal)){
      ...
      message("no palette given or too few colors for plotting by goal; using a predefined color palette from 'function: apply_bhi_theme'\n")
    } else {
...
#SKP hack to assign 90/10 split for graphical plot
    ...
    plot_config$weight[plot_config$goal == "FIS"] <- 0.9
    plot_config$weight[plot_config$goal == "MAR"] <- 0.1
    ...
    #color2 #raw colour palette
    col <- plot_df$color #data from table
...
#print(plot_obj)
      ...
      temp_plot <- paste0("temp/", "flowerplot_", name_and_title$name, ".png")
...
#hack curved labels from here   
      
#----------------------------#     
      #if(labels %in% c("curved", "arc")){
        #temp_labels <- file.path(dir_baltic, "temp", paste0("flower_curvetxt_", name_and_title$name, ".jpg"))
#skp        
        temp_labels <- paste0("temp/", "flower_curvetxt_", name_and_title$name, ".jpg")
        if(!file.exists(temp_labels)){ # don't recreate curved labels if already exist....
          circ_df <- plot_df0 %>%
            dplyr::select("goal", "f1", "f2", "name_supra", "weight", "order_calculate") %>% #keep: goal, name_supra, weight, order_calculate
...
#read in and mosaic curved labels with plot
...
#skp
        #temp_legend <- paste0("temp/", "flowerlegend_by_", color_by, "_", name_and_title$name, ".jpg")
...
#skp
      save <- paste0("reports/figures/", "flowerplot_", name_and_title$name, ".png")
    #}
    #if(!is.na(save)){
      save_loc <- save
